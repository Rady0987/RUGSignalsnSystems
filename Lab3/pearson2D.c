...
void fft2D(int direction, int width, int height, double **re, double **im) {
  double **reTmp, **imTmp;
  imTmp = allocDoubleArr2D(height, width);
  reTmp = allocDoubleArr2D(height, width);
  ...
  for(int i = 0; i < width; i++) {
    for(int j = 0; j < height; j++) {
      reTmp[i][j] = re[j][i]; //reTmp[1][0] [2][0] [3][0]     re[0][1] [0][2] [0][3]
      imTmp[i][j] = im[j][i];
    }
  }

  for(int i = 0; i < width; i++) {
    fft1D(direction, height, reTmp[i], imTmp[i]);
  }
  ...
  freeDoubleArr2D(reTmp);
  freeDoubleArr2D(imTmp);
}
